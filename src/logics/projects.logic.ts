...
import { IProjectTechRequest, IProjectRequest, ProjectResult, ProjectTechResult, } from '../interfaces/projects.interfaces'
...
const createProject = async (req: Request, res: Response): Promise<Response> => {
    try {
        ...
        const keys = requiredKeys.filter((event: any) => event[1] === undefined)
        keys.splice(
            keys.findIndex((event: any) => event[0] === 'endDate'),
            1
        )
        if (keys.length > 0) {
            const message: any[] = []
            keys.forEach((event: any) => message.push(event[0]))
...
const createProjectTech = async (req: Request, res: Response): Promise<Response> => {
    try {
...
const deleteProjectTech = async (req: Request, res: Response): Promise<Response> => {
    const projectId: number = parseInt(req.params.id)
...
export { createProject, listProjects, listProjectById, createProjectTech, deleteProject, deleteProjectTech, updateProject }